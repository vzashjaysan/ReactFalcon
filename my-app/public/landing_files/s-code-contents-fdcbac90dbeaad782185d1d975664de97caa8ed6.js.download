...
s.account="	infosysinfosysdev" //For Live site, this should point to tataul-dev
s.visitor = Visitor.getInstance("144E0EF55657094C7F000101@AdobeOrg");
/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="infosys"
s.trackingServer="infosys.sc.omtrdc.net"
s.trackingServerSecure="infosys.sc.omtrdc.net";
/************************** CONFIG SECTION **************************/
...
{
	if(s.pageName)s.eVar3=s.prop3='D=pageName'
	if(s.channel)s.eVar4=s.prop4='D=ch'
	s.prop14 = s.getTimeParting('s','+5.5');
	/* Visitor Status Tracking */
	s.eVar13=s.getNewRepeat(365);
	/* External Campaigns*/
	if(!s.campaign){
	s.campaign=s.Util.getQueryParam('soc')||s.Util.getQueryParam('cmpid')||s.Util.getQueryParam('elq')||s.Util.getQueryParam('elqaid')||s.Util.getQueryParam('eml')||s.Util.getQueryParam('gclid');
	} 
	...
	if(!s.eVar9){
		...
		s.eVar9=s.prop9=s.Util.getQueryParam('icid');

	}
	s.eVar9=s.prop9=s.getValOnce(s.eVar9,'s_ev9',0);
	if(s.eVar9!=""){
		s.events+=",event2";
  ...
  /*Previous Page Value */
	...
	s.prop5=s.getPreviousValue(s.pageName,'gpv_c6','');
...
try {
  ...
  if (window.sessionStorage) {
        ...
        s.contextData.s_dmdbase_custom = customDimensions;            
  } 
}
catch (e) {
